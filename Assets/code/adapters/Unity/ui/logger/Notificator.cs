...
public class Notificator : MonoBehaviour
{
    ...
    private void ProcessException(Exception e)
    {
        if(e is GraphMasterException)
        {
            ...
            {
                RunNotify("Невозможно сменить состояние графа на непараллельный. в рафе присутствуют паралельные ребра.", "W");
            }
            if(e is ImpossibleToSetGraphDirection)
            {
                RunNotify("Невозможно изменить состояние графа на неориентированный. Возможно в графе есть ребра которые будут считаться паралельными если убрать направленность", "W");
            }
